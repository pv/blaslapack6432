...
def generate_signatures(signatures):
    ...
    preamble = """
        ...
        #define BLASLAPACK6432_INT int
        #define BLASLAPACK6432_SRC_INT int64_t
        #define BLASLAPACK6432_EXPAND(prefix,name,suffix) prefix ## name ## _ ## suffix
        #define BLASLAPACK6432_HELPER(prefix,name,suffix) BLASLAPACK6432_EXPAND(prefix,name,suffix)
        #define BLASLAPACK6432_FUNC(name) BLASLAPACK6432_HELPER(BLASLAPACK6432_PREFIX,name,BLASLAPACK6432_SUFFIX)
        #define BLASLAPACK6432_SRC_FUNC(name) BLASLAPACK6432_HELPER(BLAS_SYMBOL_PREFIX,name,BLAS_SYMBOL_SUFFIX)

        typedef struct { float re; float im; } blaslapack6432_complex_t;
        typedef struct { double re; double im; } blaslapack6432_double_complex_t;
        """
...
def generate_code(item, prototype=False):
    ...
    template_src = """
       ...
       {%- for arg in args -%}
           ...
           size_t {{arg.name}}
           {%- else -%}
           {{ format_dst_type(arg.typespec) }} *{{arg.name}}
           {%- endif -%}
       {%- endfor -%}
       )
       {%- if prototype -%}
       ...
       {%- else %}
       {
         {%- for arg in integer_args %}
           {% if arg.dimension -%}
             {{ format_src_type(arg.typespec) }} {{arg.name}}_tmp[{{ format_array_size(arg.dimension) }}];
           {%- else -%}
             {{ format_src_type(arg.typespec) }} {{arg.name}}_tmp[1] = { *{{arg.name}} };
           {%- endif -%}
         {% endfor %}
         ...
         {% for arg in integer_args -%}
           {%- if arg.dimension %}
           for (idx = 0; idx < {{ format_array_size(arg.dimension) }}; ++idx) {{arg.name}}_tmp[idx] = ({{ format_src_type(arg.typespec) }}){{arg.name}}[idx];
           {%- endif %}
         {%- endfor -%}
         {%- if item.block == "function" %}
         ...
         {%- for arg in integer_args -%}
           {%- if arg.dimension %}
           for (idx = 0; idx < {{ format_array_size(arg.dimension) }}; ++idx) {{arg.name}}[idx] = ({{ format_dst_type(arg.typespec) }}){{arg.name}}_tmp[idx];
           {%- else %}
           {{arg.name}}[0] = ({{ format_dst_type(arg.typespec) }}){{arg.name}}_tmp[0];
           {%- endif %}
         {%- endfor %}
         {%- if item.block == "function" %}
           return return_value;
    ...
    fmt = {
        ...
        "real": "float",
        "complex": "blaslapack6432_complex_t",
        "complex*16": "blaslapack6432_double_complex_t",
    }

    def format_src_type(typespec):
        return dict(
            fmt, integer="BLASLAPACK6432_SRC_INT", logical="BLASLAPACK6432_SRC_INT",
        ).get(typespec, "void")

    def format_dst_type(typespec):
        return dict(
            fmt, integer="BLASLAPACK6432_INT", logical="BLASLAPACK6432_INT"
        ).get(typespec, "void")

    if prototype:
        BLAS_FUNC = "BLASLAPACK6432_SRC_FUNC"
        format_dst_type = format_src_type
    else:
        BLAS_FUNC = "BLASLAPACK6432_FUNC"

    BLAS_SRC_FUNC = "BLASLAPACK6432_SRC_FUNC"
    ...
    def format_array_size(dim_info):
        ...
        if value_type == "min":
            return "((*{0}) < (*{1}) ? (*{0}) : (*{1}))".format(*value)
        elif value_type == "mulmin":
            return "{0} * ((*{1}) < (*{2}) ? (*{1}) : (*{2}))".format(*value)
        else:
